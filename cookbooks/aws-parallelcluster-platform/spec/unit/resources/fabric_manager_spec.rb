        ...
        if platform == 'ubuntu'
          ...
          it 'installs fabric manager' do
            is_expected.to run_bash("Install nvidia-fabric-manager")
              ...
              .with_retry_delay(5)
              .with(code: %(    set -e
    aws s3 cp #{node['cluster']['artifacts_build_url']}/nvidia_fabric/#{platform}/#{fabric_manager_package}-#{fabric_manager_version}-1.x86_64.rpm #{fabric_manager_package}-#{fabric_manager_version}.rpm --region #{node['cluster']['region']}
